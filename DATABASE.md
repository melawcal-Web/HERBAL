...
| --- | --- | --- |
| **שינויים בסכימה** (טבלאות, עמודות, אינדקסים לפי `prisma/schema.prisma`) | כן — אחרי הבילד, ב־**pre-deploy** רץ `prisma db push` (בלי `--accept-data-loss`) | הבילד עצמו לא מתחבר ל-MySQL (הרשת הפנימית לא זמינה שם). אם הסנכרון נכשל בגלל שינוי הרסני — לתקן את הסכמה במפורש, לא לדרוס נתונים בשקט. |
