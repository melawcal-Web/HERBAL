...
import { setTherapistProductActive } from "@/app/actions/therapist-product";

export function TherapistProductActiveToggle({
  productId,
  active,
  title,
}: {
      ...
      <button
        ...
        onClick={() => {
          ...
          const ok = window.confirm(
            next ? `להפעיל מחדש את «${title}»?` : `להשבית את «${title}»? הפריט יוסתר מהדף הציבורי.`,
          );
          if (!ok) return;
          setError(null);
          startTransition(async () => {
            try {
              await setTherapistProductActive(productId, next);
              router.refresh();
            } catch (e) {
