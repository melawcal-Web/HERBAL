...
export function HomeVisionCarousel({ slides }: { slides: VisionSlide[] }) {
  ...
  useEffect(() => {
    if (n <= 1 || reducedMotion) return;
    const id = window.setInterval(beginAdvance, 7000);
  ...
  return (
    ...
    >
      <div className="relative min-h-[min(52vh,440px)] w-full sm:min-h-[460px]">
        <div
        ...
        <div className="relative z-10 flex min-h-[min(52vh,440px)] flex-col justify-center px-5 py-10 text-right sm:min-h-[460px] sm:px-10 sm:py-12">
          {current.eyebrow && (
            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-herbal-700/90 sm:text-xs">
              {current.eyebrow}
            </p>
          )}
